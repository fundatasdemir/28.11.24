...
class Bolunme extends StatefulWidget {
  const Bolunme({super.key});

  @override
  _BolunmeState createState() => _BolunmeState();
}

class _BolunmeState extends State<Bolunme> {
  List<String> bolunmeInfo = [];
  ...
  Future<void> fetchBolunmeInfo() async {
    try {
      final response = await http
          .get(Uri.parse('https://www.kutso.org.tr/wp-json/wp/v2/pages/3659'));

      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        setState(() {
          bolunmeInfo = parseHtmlString(data['content']['rendered']);
        });
      } else {
        throw Exception('Failed to load Bolunme info');
      }
    } catch (e) {
      print('Error fetching data: $e');
      setState(() {
        bolunmeInfo = ['Bilgiler yüklenirken bir hata oluştu.'];
      });
    }
  }
