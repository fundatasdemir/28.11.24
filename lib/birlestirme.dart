...
class Birlestirme extends StatefulWidget {
  const Birlestirme({super.key});

  @override
  _BirlestirmeState createState() => _BirlestirmeState();
}

class _BirlestirmeState extends State<Birlestirme> {
  List<String> birlestirmeInfo = [];
  Map<String, String> pdfLinks = {};

  @override
  void initState() {
    super.initState();
    fetchBirlestirmeInfo();
  }

  Future<void> fetchBirlestirmeInfo() async {
    try {
      final response = await http
          .get(Uri.parse('https://www.kutso.org.tr/wp-json/wp/v2/pages/3657'));

      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        setState(() {
          birlestirmeInfo = parseHtmlString(data['content']['rendered']);
        });
      } else {
        throw Exception('Failed to load Birlestirme info');
      }
    } catch (e) {
      print('Error fetching data: $e');
      setState(() {
        birlestirmeInfo = ['Bilgiler yüklenirken bir hata oluştu.'];
      });
    }
  }
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: birlestirmeInfo.map((info) {
              final isClickable =
