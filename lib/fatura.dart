...
class _FaturaState extends State<Fatura> {
  ...
  Future<void> fetchFaturaInfo() async {
    final response = await http
        .get(Uri.parse('https://www.kutso.org.tr/wp-json/wp/v2/pages/3738'));

    if (response.statusCode == 200) {
      var data = jsonDecode(response.body);
      setState(() {
        faturaWidgets = parseHtmlString(data['content']['rendered']);
      });
    } else {
      throw Exception('Failed to load Fatura info');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Fatura Tasdiki'),
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: faturaWidgets.isEmpty
                ? [
                    const Center(
