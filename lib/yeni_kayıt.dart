...
class _YeniKayitState extends State<YeniKayit> {
  ...
  Future<void> fetchYeniKayitInfo() async {
    try {
      final response = await http
          .get(Uri.parse('https://www.kutso.org.tr/wp-json/wp/v2/pages/3653'));

      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        setState(() {
          yeniKayitInfo = parseHtmlString(data['content']['rendered']);
        });
      } else {
        throw Exception(
            'Yeni Kayıt bilgileri yüklenemedi. HTTP ${response.statusCode}');
      }
    } catch (e) {
      print('Hata: $e');
      setState(() {
        yeniKayitInfo = ['Bilgiler yüklenirken bir hata oluştu.'];
      });
    }
  }
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: yeniKayitInfo.map((info) {
              final isClickable =
